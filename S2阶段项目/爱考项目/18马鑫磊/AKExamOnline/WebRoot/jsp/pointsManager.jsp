...
function showPointsList(pid) {

	var width = (screen.width - 840) / 2;

	var height = (screen.height - 720) / 2;

	var res = window

			.open(

					"../paper/paper_getPointsTestList?paper.pid="+pid,

					"window",

					"width=840px,height=720px,top="
...
<body>
	...
	<div>
		...
		<form action="paper/paper_getPaperPointsList" method="post">
		...
		<fieldset id="marginTop">
				...
				<s:iterator value="pageBean.Items" var="paperinfo" status="status">
					...
					<tr>
						...
						<td><s:if test="#paperinfo.pstate == '未开考'">
							...
							</s:else> <a href="javascript:showPointsList(${paperinfo.pid })">查看试卷</a></td>
			...
			<div align="right" id="marginTop">
				...
				第${pageBean.currentIndex }页， 共${pageBean.totalPage }页&nbsp;&nbsp; <a
					...
					href="paper/paper_getPaperPointsList?course.csId=${course.csId }&paper.ptype=${paper.ptype}&paper.pstate=${paper.pstate }&pageIndex=1">首页</a>&nbsp;

				<a

					href="paper/paper_getPaperPointsList?course.csId=${course.csId }&paper.ptype=${paper.ptype}&paper.pstate=${paper.pstate }&pageIndex=${pageBean.preIndex }">上一页</a>&nbsp;

				<a

					href="paper/paper_getPaperPointsList?course.csId=${course.csId }&paper.ptype=${paper.ptype}&paper.pstate=${paper.pstate }&pageIndex=${pageBean.nextIndex }">下一页</a>&nbsp;

				<a

					href="paper/paper_getPaperPointsList?course.csId=${course.csId }&paper.ptype=${paper.ptype}&paper.pstate=${paper.pstate }&pageIndex=${pageBean.totalPage }">末页</a>
