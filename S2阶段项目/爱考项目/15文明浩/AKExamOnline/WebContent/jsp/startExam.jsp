...
<script type="text/javascript">
	function addTr() {
		...
		cell.innerHTML = "<input type='button' value='删除' onclick=\"deleteRow('rid"
				+ trNo + "')\">";
		//设置select标签的数据
	...
	function updateTr() {
		...
		$("tr").each(
				function(index, element) {
					$($(":hidden").get(index)).attr("name",
							"paperClasses[" + index + "].pid");
					$($(":select").get(index)).attr("name",
							"paperClasses[" + index + "].ccode");
					$($(":Wdate").get(index)).attr("name",
							"paperClasses[" + index + "].examDate");
					$($(":endTime").get(index)).attr("name",
							"paperClasses[" + index + "].endDate");
...
<body>
	<form action="paper/paper_startExam" method="post">
		...
		<table id="tab_exam" align="center" cellspacing="1px">
			...
			<tr>
				<td><select name="paperClasses[0].ccode">
						<s:iterator value="listClazz" var="clazz">
							<option value="${clazz.ccode }">${clazz.cname }</option>
						</s:iterator>
				</select></td>
				<td><input type='text' class="Wdate" id='dateTime'
					name='paperClasses[0].examDate'
					onfocus="WdatePicker({skin:'whyGreen',dateFmt: 'yyyy-MM-dd HH:mm',lang:'zh-cn', minDate: '%y-%M-%d 08:00}' })" />
				</td>
